      ...
      <div className='w-full'>
        {items.map((item, index) => (
          <div key={index} className={cn('pb-4')}>
            ...
            {item?.items?.length && (
              <DocumentationSidebarItem items={item.items} pathname={pathname} />
            )}
          </div>
        ))}
      </div>
...
interface DocumentationSidebarItemProps {
  items: SidebarNavItem[];
  pathname: string | null;
}

export function DocumentationSidebarItem({ items, pathname }: DocumentationSidebarItemProps) {
  return (
    items?.length && (
