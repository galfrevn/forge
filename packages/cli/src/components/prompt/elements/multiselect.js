...
import clear, { strip } from '@/components/prompt/utils/clear';
  ...
  render() {
    ...
    if (this.done) {
      this.outputText.push(
        this.choices
          .map((choice) => (choice.selected ? `${prefix} ${color.dim(`${choice.label}`)}\n` : ''))
          .join('')
          .trimEnd()
      );
    } else {
          ...
          .map((choice, i) =>
            i === this.cursor
              ? `${prefix.slice(0, -2)}${color.cyanBright('▶')}  ${
                ...
                } ${color.underline(choice.label)} ${choice.hint ? color.dim(choice.hint) : ''}`
